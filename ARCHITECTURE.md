...
> *"O serviço de controle de entry NÃO PODE ficar indisponível se o sistema de daily consolidation cair."*
...
| `CashFlow.Transactions.API` | Receives transactions, grava no DB | Bus Outbox + Delivery Service | Confia nos Headers do YARP (`X-User-Id`) |
...
| RT-2 | Testes | ✅ Unit, Integration, Architecture, Load, Contract, **E2E (Aspire)** |
...
*   **Value:** A quantia monetária (sempre positiva) associada a um entry.
...
A `ConsumerDefinition` configura `UseMessageRetry` com backoff (100ms, 500ms, 1s, 2s, 5s) seguido de `UseEntityFrameworkOutbox` como middleware mais interno (gerencia a TX que envolve o consumer). A versao canonica de producao (ADR-005) adiciona `UsePartitioner`, `UseDelayedRedelivery` e retry exponencial com jitter.
...
O `azd` lê o AppHost e provisiona toda a infraestrutura automaticamente: `azd provision` gera Bicep in-memory a partir do AppHost e provisiona os recursos Azure; `azd deploy` builda as imagens via `dotnet publish /t:PublishContainer` e faz deploy para ACA. Não há Dockerfiles nem manifests manuais — o `azd` gera tudo a partir do AppHost. Veja [docs/ci-pipeline.md seção 10](./ci-pipeline.md#10-cd--deploy-para-azure-container-apps-via-azd) para detalhes do workflow de CD.
...
O teste cria um credit de R$ 250,00 via Gateway, aguarda a consistencia eventual via polling (500ms de intervalo, 15s de timeout) e verifica que o `TotalCredits` e o `TransactionCount` refletem o entry criado. Um segundo teste complementar cria um debit de R$ 100,00 e verifica que o `TotalDebits` aumentou corretamente, cobrindo o caminho alternativo do `ApplyTransaction`.
...
Os smoke tests de health check validam que todos os servicos (Gateway, Transactions, Consolidation, Identity) respondem 200 OK em seus respectivos endpoints de saude (`/health/live`, `/health/ready`, `/health`). Cada teste cria um HttpClient via `fixture.App.CreateHttpClient(serviceName)` usando o service discovery interno do Aspire. Adicionalmente, um teste verifica que o YARP retorna 401 Unauthorized para requisicoes sem Bearer Token, validando o Auth Offloading (ADR-006).
...
| **Worker crash** | Mensagens ficam na fila (unacked) | RabbitMQ redistribui ao reiniciar |
| **DB do consolidation fora** | Consumer falha no commit | `UseMessageRetry` com backoff exponencial |
| **DB do entry fora** | API retorna 503 | Circuit breaker abre → erro imediato |
...
> *"O serviço de entry não deve ficar indisponível se o consolidation cair."*

**Prova**: A API de Transactions depende de `{PostgreSQL Write, RabbitMQ}`. O Projection Worker e a API do Consolidation **não** estão nesse conjunto. Logo, a queda de qualquer componente do consolidation não afeta a transaction.
...
| Npgsql connection pool (100 conns, ~30ms/query) | ~3.300 req/s | **66x** |
...
3. **Pool sizing explícito:** `Minimum Pool Size=2; Maximum Pool Size=20` na connection string do Consolidation.API. Com cache hit rate de ~80%, apenas ~10 req/s chegam ao PostgreSQL; `10 × 0,003s = 0,03` conexões concorrentes no regime estável — pool de 20 representa margem de 650x.
...
| Causa | Taxa Esperada |
|---|---|
| Timeout de pool | ~0% (margem de 66x) |
...
> **Nota:** O throughput do consumer MassTransit (processamento de eventos de entry) é uma dimensão **ortogonal**: governa a latência de consistência eventual do consolidation, não sua capacidade de leitura. Ver seção NFR-4 abaixo.
...
Esta seção é ortogonal ao NFR-2: governa **quanto tempo após um entry ser criado o consolidation reflete o novo balance** — não a capacidade de leitura.
...
**Configuracao otimizada do Bus Outbox (R2 — MEDIO):** A configuracao otimizada define `QueryDelay = 100ms` (reduzido do padrao de 1000ms), `MessageDeliveryLimit = 100` (maximo de mensagens por ciclo de poll) e `DuplicateDetectionWindow = 30 minutos` (aumentado do padrao de 30s para cobrir janelas de retry maiores). O lock provider usa PostgreSQL advisory locks via `UsePostgres()`.
...
|---|---|---|
| **Traces** | OpenTelemetry → Jaeger | Rastreamento do HTTP request até o consumer |
| **Metrics** | OpenTelemetry → Prometheus/Grafana | Transactions/s, latência consumer, queue depth |
| **Logs** | Serilog → OpenTelemetry Collector | Logs estruturados com TraceId/SpanId |
| **Health** | ASP.NET HealthChecks | `/health/live`, `/health/ready` (DB + RabbitMQ) |
| **Dashboard** | .NET Aspire Dashboard | Visualização integrada em dev |
...
Para um sistema financeiro, a ausência de backup e Point-in-Time Recovery (PITR) é uma lacuna crítica de governança de dados. A LGPD (Lei 13.709/2020) e as normas do Banco Central do Brasil exigem rastreabilidade e recuperabilidade de dados financeiros. A estratégia abaixo cobre retenção mínima de 90 dias.

#### Objetivos de Recuperação

| Métrica | Definição | Meta |
|---|---|---|
| **RPO** (Recovery Point Objective) | Perda máxima de dados aceita | < 5 minutos (ultimo segmento WAL arquivado) |
| **RTO** (Recovery Time Objective) | Tempo máximo de recuperação | < 4 horas |
| **Retenção** | Periodo mínimo de backup disponível | 90 dias (mínimo regulatório LGPD + BCB) |

#### Camada 1 — WAL Archiving Contínuo (RPO < 5 min)

O WAL (Write-Ahead Log) do PostgreSQL registra cada transação antes de ser aplicada. Com archiving habilitado, cada segmento WAL (~16 MB) é copiado para storage durável imediatamente após ser completado, garantindo granularidade de segundos para recuperação. A configuracao requer `wal_level = replica`, `archive_mode = on` e `archive_command` apontando para storage local ou remoto (S3, Azure Blob). O `archive_cleanup_command` garante que WAL segments nao sejam removidos antes de serem arquivados.

#### Camada 2 — Backup Base Semanal (pg_basebackup)

O backup base e o ponto de partida para qualquer recuperacao PITR. Ele captura o estado consistente do cluster PostgreSQL completo. O script de backup semanal executa via cron (domingo 02:00), usando `pg_basebackup` com formato tar comprimido e `--checkpoint=fast`. Apos completar o backup, remove automaticamente backups base e WAL segments com mais de 90 dias para manter a retencao dentro dos limites configurados.

#### Camada 3 — PITR (Point-in-Time Recovery)

Para recuperar o banco para um ponto específico no tempo (ex: imediatamente antes de uma exclusão acidental ou corrupção de dados), o procedimento envolve 6 passos: (1) parar o PostgreSQL, (2) restaurar o backup base mais recente anterior ao ponto desejado, (3) configurar `restore_command`, `recovery_target_time` e `recovery_target_action = 'promote'` no `postgresql.conf`, (4) criar o arquivo `recovery.signal`, (5) iniciar o PostgreSQL (que aplicara os WAL segments ate o target time), e (6) verificar nos logs se a recuperacao foi bem-sucedida.

#### Schedule Consolidation de Backups

```
Domingo 02:00 UTC   ── pg_basebackup (full, retencao 90 dias)
Segunda a Sabado    ── WAL archiving continuo (por segmento ~16MB, retencao 90 dias)
                                    ↓
Granularidade de recuperacao: qualquer ponto nos ultimos 90 dias com precisao de segundos
```

#### Monitoramento de Saúde do Backup

Dois alertas Prometheus monitoram a saude do backup: **WALArchivingFalhou** (severity: critical) dispara se `pg_stat_archiver_failed_count > 0` por mais de 10 minutos, indicando que o RPO esta comprometido. **BackupMuitoAntigo** (severity: warning) dispara se o ultimo backup base bem-sucedido foi ha mais de 8 dias, indicando falha no cron ou falta de espaco em disco.

O monitoramento do WAL archiving e feito via `pg_stat_archiver`, verificando `archived_count`, `last_archived_time` e `failed_count`. Um alerta e disparado se `seconds_since_last_wal` exceder 300 segundos (5 minutos = limite do RPO).

#### Teste de Restore (Obrigatório — Mensal)

Um backup nao testado nao e um backup. Procedimento de teste mensal:

1. Provisionar uma instancia PostgreSQL isolada (container ou VM separada).
2. Restaurar o backup base mais recente.
3. Aplicar WAL segments ate um ponto no tempo do dia anterior.
4. Executar queries de validação: contagem de transactions, verificação de balances de referência.
5. Documentar o RTO real medido (tempo total do Passo 1 ao 4).
6. Registrar resultado no log de auditoria de backup.

#### Alternativas Managed (Producao Real)

Para ambientes em nuvem, servicos gerenciados oferecem PITR nativo sem configuracao manual de WAL archiving:

| Servico | PITR nativo | Retencao maxima | Observacao |
|---|---|---|---|
| **AWS RDS PostgreSQL** | Sim — ate 35 dias | 35 dias (configuravel) | Backup automatico diario + WAL continuo |
| **Azure Database for PostgreSQL** | Sim — ate 35 dias | 35 dias | Geo-redundant backup opcional |
| **Supabase** | Sim (planos pagos) | 7-30 dias | Interface simples, ideal para startups |
| **Neon** | Sim — branching por ponto no tempo | Variavel por plano | Modelo serverless, PITR por branch Git-like |
| **pg_basebackup manual** (documentado acima) | Implementacao propria | 90 dias (configuravel) | Controle total, custo de operacao |

> **Recomendacao para producao**: Em ambiente cloud, usar AWS RDS ou Azure Database for PostgreSQL com PITR nativo (configurar retencao para 35 dias e complementar com snapshot manual mensal retido por 90 dias para conformidade regulatoria). Em ambiente on-premise ou self-hosted, implementar a estrategia WAL archiving + pg_basebackup documentada acima.
...
| **Contrato** | PactNet (Consumer-Driven Contract Testing) | `CashFlow.IntegrationTests` | Garante que o evento `TransactionCreated` publicado pelo produtor (Transactions.API) é compatível com o schema que o consumidor (Consolidation.API) espera deserializar |
| **Carga** | NBomber / k6 | Pipeline separado | Validation empírica de 50 req/s (NFR-2, NFR-4) |
...
```
        ...
        / Contrato\     PactNet — contratos de eventos
       /────────────\
...
> **Por que testes de contrato são críticos em arquiteturas event-driven**: Uma renomeação de campo no evento `TransactionCreated` (ex: `MerchantId` → `TenantId`) pode passar em todos os testes unitários e de integração sem ser detectada — cada serviço testa seu lado isoladamente. O PactNet gera um "pact file" no lado consumidor descrevendo o contrato esperado, que o produtor deve verificar antes de qualquer deploy. Isso cria uma barreira contra breaking changes silenciosos no schema do evento.

Os testes de contrato utilizam PactNet no lado consumidor (Consolidation.API) para definir o schema esperado do evento `TransactionCreated`, garantindo que campos obrigatorios (`MerchantId`, `ReferenceDate`, `Type`, `Value`) estejam presentes e com tipos corretos. O produtor (Transactions.API) verifica o pact file gerado antes de qualquer deploy.
...
| `GET` | `/health/live` | Liveness check | Anônimo |
| `GET` | `/health/ready` | Readiness check | Anônimo |
